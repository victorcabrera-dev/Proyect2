            ...
            if self.cont_frame == 48:
                ...
                if len(faces_database) != 0 and not self.comparison and self.matcher is None:
                    ...
                    if self.matcher:
                        ...
                        return face_image, self.matcher, 'Acceso Aprobado!'
                    else:
                        return face_image, self.matcher, 'Acceso no aprobado'
                else:
                    return face_image, self.matcher, 'Base de datos vacia'
            else:
                return face_image, self.matcher, 'wait frames'
        else:
            return face_image, self.matcher, 'El rostro no esta centrado'
